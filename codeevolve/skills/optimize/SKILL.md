---
name: optimize
description: Evolve a Python function to be faster using CodeEvolve. Use when the user wants to optimize, speed up, or improve the performance of a Python function.
disable-model-invocation: true
argument-hint: <function-file> <test-file> [--iterations N] [--function <name>] [--yes]
---

# Optimize

Run CodeEvolve to evolve a Python function to be faster.

The user's message contains the arguments after `/codeevolve:optimize`. Parse them:
...
- `--objective time` (only supported value in v1.0; if user passes `space` or `balanced`, tell them these are deferred to v2.0 and proceed with `time`)
...
Report no significant improvement found. Suggest trying `--iterations 50`.
